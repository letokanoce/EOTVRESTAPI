...
class RedisHandler:
    ...
    def get(self, base_profile: BaseProfile):
        ...
        try:
            entity_exists, entity = self._level_exists_in_cache(self.client, key, base_profile)
    ...
    def set(self, base_profile: BaseProfile, strategy: ISubStrategy):
        ...
        try:
            if not self.client.exists(key):
                self.client.hset(key, base_profile.sub_class_level,
                                 self.serializer.serialize(base_profile))
    ...
    def _level_exists_in_cache(self, client, key, base_profile):
        values = client.hvals(key)
        matched_entity = [self.serializer.deserialize(val) for val in values if
                          self.serializer.deserialize(val).sub_class_level == base_profile.sub_class_level]
        entity_exists = any(matched_entity)
        return entity_exists, matched_entity[0] if entity_exists else None

    def _add_or_replace_value(self, client, key, base_profile, strategy):
        values = client.hvals(key)
        if len(values) < 2:
            client.hset(key, base_profile.sub_class_level, self.serializer.serialize(base_profile))
    ...
    def _convert_to_json(self, base_profile: BaseProfile):
        entity_bytes = self.serializer.serialize(base_profile)
